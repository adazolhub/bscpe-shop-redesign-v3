...
const ListGrid = ({ data }) => {
  ...
  suspend(getInitialAuthState, "log");
  return (
    <>
      ...
      {!currentUser ? (
        ...
        <WithProps data={data} currentUser={currentUser} />
      )}
    </>
  );
};
...
function WithProps({ data, currentUser = null }) {
  ...
  useEffect(() => {
    let qUser =
      currentUser &&
      query(userRef, currentUser && where("uid", "==", currentUser?.uid));
    let userUnsub =
      currentUser &&
  ...
  return (
    <>
      <main className="lg:col-span-2 lg:order-2 lg:row-span-1" id="list">
        {/* //this props need to move to global context props */}
        <Tabs toggleTabs={toggleTabs} handleToggleTabs={handleToggleTabs} />
              ...
              data?.map((product) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 w-[calc(100%-1em)]">
                    <h3 className="px-2 mb-2 text-xs font-medium text-white sm:text-sm md:text-lg">
                      {product?.product_name}
                    </h3>
                    ...
                    <div className="flex w-full gap-2 mx-2 mb-2 flex-nowrap">
                      ...
                      <button
                        className="p-1 px-2 text-sm text-gray-100 border rounded-md whitespace-nowrap border-gray-300/10 bg-gray-100/5 w-fit backdrop-blur-sm"
...
function ListGridToggleButton({ toggleList, handleToggleList }) {
  return (
...
function Tabs({ toggleTabs, handleToggleTabs }) {
  return (
