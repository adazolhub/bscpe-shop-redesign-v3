...
const ProductCard = ({ product_id, name, image, price, ...props } : any) => {
  let { products, addToCart, removeFromCart } : any = ShopState();
  let [isInCart, setIsInCart] = useState(false);

  useEffect(() => {
    let productIsInCart = products.find((product: any) => product.name === name);
        ...
        <div className="absolute bottom-0 w-[calc(100%-1em)] mx-2">
          ...
          Buy now
            ...
            <button
              ...
              onClick={() => {
                toggleItemHandler()
                setSelectedProduct((prev : any) => prev = { product_id, product_name: name, product_image: image, product_price: price })
              }}
            >
              {isInCart ? (
